...
// capacidadRedimension Devuelva la nueva capacidad para redimensionar la tabla y -1 en caso de que no haya que redimensionarla
func capacidadRedimension(capacidadDiccionario, cantidadElementos int) int {

	var nuevaCapacidad int
	factorRedimension := float64(cantidadElementos) / float64(capacidadDiccionario)

	if factorRedimension < FACTOR_CARGA_MINIMO {
		nuevaCapacidad = capacidadDiccionario / DIVISOR_NUEVA_CAPACIDAD
		if nuevaCapacidad < CAPACIDAD_INICIAL {
			return NO_REDIMENSIONAR
		}
		return nuevaCapacidad

	} else if factorRedimension > FACTOR_CARGA_MAX {
		return capacidadDiccionario * MULTIPLICADOR_NUEVA_CAPACIDAD
	}
...
func redimensionar[K comparable, V any](diccionario *hashAbierto[K, V]) {
	nuevaCapacidad := capacidadRedimension(cap(diccionario.tabla), diccionario.cantidad)
	if nuevaCapacidad == NO_REDIMENSIONAR {
		return
	}
...
// buscarEnLista itera una lista buscando donde esta la clave mandada, si la encuentra devuelve el iterador, el indice de la tabla y true, en caso contrario false
func buscarEnLista[K comparable, V any](diccionario *hashAbierto[K, V], clave K) (TDALista.IteradorLista[par[K, V]], int, bool) {
	indice := obtenerIndice(clave, cap(diccionario.tabla))
	iter := diccionario.tabla[indice].Iterador()

	for iter.HaySiguiente() {
		if iter.VerActual().clave == clave {
			return iter, indice, true
		}
		iter.Siguiente()
	}

	return iter, indice, false
}

func (diccionario *hashAbierto[K, V]) Guardar(clave K, valor V) {
	redimensionar(diccionario)

	iter, indice, pertenece := buscarEnLista(diccionario, clave)
	if pertenece {
		iter.Borrar()
		iter.Insertar(par[K, V]{clave: clave, valor: valor})

	} else {
		diccionario.tabla[indice].InsertarUltimo(par[K, V]{clave: clave, valor: valor})
		diccionario.cantidad++
	}
}

func (diccionario *hashAbierto[K, V]) Pertenece(clave K) bool {
	_, _, pertenece := buscarEnLista(diccionario, clave)
	return pertenece
}

func (diccionario *hashAbierto[K, V]) Obtener(clave K) V {
	iter, _, pertenece := buscarEnLista(diccionario, clave)
	if pertenece {
